...
# =========================================================================
# SYNTHETIC-ONLY HELPERS (active when config["synthetic"]["enabled"] is True)
# =========================================================================
def chebyshev_true_background(x, coeffs, x_full):
    """
    Evaluate a GSAS-II 'chebyschev-1' background at positions ``x``.

    Faithful to GSAS-II's convention: the abscissa is mapped onto [-1, 1]
    using the FULL (unmasked) pattern endpoints,

        t = -1 + 2 * (x - x_full[0]) / (x_full[-1] - x_full[0]),

    and the background is the first-kind Chebyshev series

        b(x) = sum_n c_n * cos(n * arccos(t)) = sum_n c_n * T_n(t),

    which is exactly numpy's chebval(t, coeffs). ``coeffs`` is the list
    [c0, c1, ...] in GSAS-II order.

    IMPORTANT: ``x`` and ``x_full`` must be the SAME abscissa GSAS-II fit on
    (the native 2theta / centidegree scale), NOT Q. The mapping is affine so
    it is invariant to the centidegree<->degree rescaling, but Q is a
    nonlinear transform and would give a different (wrong) background.
    """
    coeffs = np.asarray(coeffs, dtype=float)
    x = np.asarray(x, dtype=float)
    x0 = float(x_full[0])
    dt = float(x_full[-1]) - x0
    t = -1.0 + 2.0 * (x - x0) / dt
    return chebval(t, coeffs)


def r_pattern_percent(y_true, y_calc):
    """
    Unweighted profile R-factor, in percent:

        R_p = 100 * sum_i |y_true_i - y_calc_i| / sum_i |y_true_i|.

    Here y_true is the known synthetic background and y_calc is the GP
    background mean, evaluated on the same (masked) points.
    """
    y_true = np.asarray(y_true, dtype=float)
    y_calc = np.asarray(y_calc, dtype=float)
    return 100.0 * np.sum(np.abs(y_true - y_calc)) / np.sum(np.abs(y_true))
...
def make_figures(x_obs, y_obs, GP_pred, GP_std, fig_dir, label, training_points,
    ...
    side-by-side subplots:
      (left)  observed pattern with fitted background envelope and a
              +/- 1 GP_std band; if ``y_true`` is provided (synthetic mode)
              the known background is overlaid as a dotted line labelled
              "True";
...
def main(config_path):
    ...
    x_full = np.asarray(x_obs, dtype=float).copy()  # native abscissa; endpoints define the GSAS-II chebyschev-1 map
    ...
    # -----------------------------------------------------------------
    # Synthetic mode: known chebyschev-1 background + R_pattern vs truth
    # -----------------------------------------------------------------
    syn_cfg = config.get("synthetic", {})
    synthetic = bool(syn_cfg.get("enabled", False))
    y_true = None
    R_pattern = None
    if synthetic:
        # Truth is mapped in the native abscissa (x_full / masked x_obs),
        # NOT in Q. Both are indexed by the same kept points.
        y_true = chebyshev_true_background(
            x_obs, syn_cfg["cheb_coeffs"], x_full,
        )
        R_pattern = r_pattern_percent(y_true, GP_pred)
        print(
            f"R_pattern (GP background mean vs truth) = {R_pattern:.4f} %",
            flush=True,
        )
    ...
    if synthetic:
        save_kwargs["y_true"] = y_true
        save_kwargs["R_pattern"] = np.asarray(R_pattern, dtype=float)
    np.savez(out_dir / "gp_fit.npz", **save_kwargs)
    if synthetic:
        np.savetxt(
            out_dir / "R_pattern.txt", [R_pattern],
            header="R_pattern (percent), unweighted profile R-factor: "
                   "GP background mean vs true chebyschev-1 background",
        )
        print(f"Saved R_pattern.txt (R_pattern={R_pattern:.4f} %).", flush=True)
